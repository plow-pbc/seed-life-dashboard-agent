...
test("in-window tick fetches, composes, and posts card 3 / type:weather", async () => {
  ...
  // text is the tile-spec JSON the kiosk renders generically
  assert.equal(body.text, res.text);
  const spec = JSON.parse(body.text);
  assert.equal(spec.rows[0].cells[0].cells[0].value, "72°");
  assert.equal(spec.rows[0].cells[0].cells[1].value, "Sunny");
...
test("--dry-run composes but never POSTs", async () => {
  ...
  assert.equal(JSON.parse(res.text).rows[0].cells[0].cells[0].value, "72°");
