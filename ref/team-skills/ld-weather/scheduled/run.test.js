...
// Kiosk is best-effort now (postKioskCard): a bad URL or a failed POST logs and
// returns posted:false rather than crashing the scheduled runner on an offline Pi.
test("non-http(s) kiosk URL is skipped best-effort (posted:false, no bearer sent)", async () => {
  for (const badUrl of ["ftp://kiosk.example/api/message", "notaurl"]) {
    ...
    assert.equal(res.posted, false, `posted:false for ${badUrl}`);
    assert.ok(typeof res.text === "string" && res.text.length > 0);
  }
});

test("a failed kiosk POST is best-effort — posted:false, never throws", async () => {
  const res = await run({
    now: new Date("2026-06-09T22:00:00Z"),
    config: baseConfig(),
