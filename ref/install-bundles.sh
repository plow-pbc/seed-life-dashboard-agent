...
API_TOKEN_FILE="$APP_SUPPORT/plow-api-token"
...
then
  ...
  for _ in $(seq 1 18); do          # ~3 min: registration rides an async agent turn
    if [ -f "$CRON_JOBS" ] \
       && n=$(jq -r '.. | .name? // empty' "$CRON_JOBS" 2>/dev/null) \
       && printf '%s\n' "$n" | grep -qx 'ld-morning-updates' \
       && printf '%s\n' "$n" | grep -qx 'ld-morning-triage' \
       && printf '%s\n' "$n" | grep -qx 'ld-weekly-digest'; then
      cron_ok=1; break
    fi
    sleep 10
  done
...
if [ -n "$cron_ok" ]; then
  echo "  crons registered: ld-morning-updates, ld-morning-triage, ld-weekly-digest (authoritative check: cron action=list)" >&2
else
  echo "" >&2
