...
1. **Dashboard secrets present and well-shaped.** Do `<app_support>/agent-runtime/secrets/dashboard-endpoint-url` and `dashboard-token` exist with mode `600` and non-zero size — AND does the endpoint file hold exactly one non-blank line matching `http(s)://…/api/message` with no whitespace, and the token file exactly one whitespace-free line (RFC 6750 bearers carry no whitespace; the same predicates the install action enforces before any plowd mutation; checked without printing either value)? Expected: yes.
