...
- **family.timezone is autodetected, not an input.** The IANA zone is everything after the last `/zoneinfo/` in `readlink /etc/localtime` (e.g. `/usr/share/zoneinfo/America/New_York` → `America/New_York`), falling back to `America/Los_Angeles` if detection yields nothing — so a non-Pacific household gets the right local time without a 4th question. The single detection rule lives in [`ref/lib/detect-timezone.sh`](ref/lib/detect-timezone.sh); install and verify source it once and feed the same detected zone to BOTH assembly (which writes it into the config) and the gate (which asserts the landed config carries the SAME zone) — so a tz-autodetect regression can't ship a wrong zone that the gate still passes.
- **The minimal install gate.** The install/verify gate is deliberately minimal — rather than mirror `run.js`'s field-by-field requirements (a list that drifted from the runtime contract across multiple review rounds), it checks only the invariants that distinguish a USABLE filled config from an unedited template or a blank-filled one:
  ...
  - `family.timezone` equals the host-autodetected zone — checked ONLY when the SEED ASSEMBLES the config (resolve_and_land passes the detected zone to the to-land gate), so a tz-autodetect regression can't ship a wrong zone in a config the SEED built. The standalone post-landing gate and verify do NOT enforce this (a preserved operator-edited / `LD_CONFIG_SRC=-`-supplied config may carry an intentionally non-host zone — after a kiosk move or for a remote household — and must not be falsely rejected), so the landing gate and the post gate never disagree.
...
- On first install (re-runs MUST NOT overwrite a gate-PASSING file — the operator's edits are canonical), the install action lands `<app_support>/agent-runtime/runtime/ld/config.json` at mode 600 by the three-way resolution in [ld-config](#ld-config): **assemble from the declared [inputs](#requirements)** (the default, when `LD_OWNER_NAME` / `LD_OWNER_IMESSAGE` / `LD_CALENDAR_ACCOUNT` are present), else **consume a supplied config from `LD_CONFIG_SRC=-`** (escape hatch), else **copy the vendored example** for manual editing. The ONE exception to re-run preservation: when the existing landed config still FAILS the minimal gate (e.g. a first run with no inputs landed the placeholder example, or a manually-corrupted file) AND a supply source (inputs or `LD_CONFIG_SRC`) is now present, the install action assembles/consumes the new config and atomically replaces the bad file through the same validation path — otherwise the early "file exists" return would short-circuit a corrected rerun and silently ignore it. The freshly-assembled config is authoritative on a corrected single-shot rerun (new owner / calendar account / host timezone). The preservation gate (path (a)) does NOT enforce the tz-match: a structurally-valid existing config is the operator's canonical edit and is kept even if its zone drifted from the current host (laptop moved, `/etc/localtime` repointed) or was hand-set — only a freshly-assembled config enforces the host zone. The `LD_CONFIG_SRC=-` escape hatch likewise TRUSTS the caller's supplied zone (a remote/headless caller may supply a config for a different host), so the tz-match check applies only to the assemble-from-inputs path.
...
- After landing (or detecting an existing) `ld-config`, the install action MUST gate the bundle POST on the minimal STRUCTURAL gate (`calendar.sources` a non-empty array with non-blank `account`s, `family.owner.{name,imessage}` non-blank, and no bare `[UPPER_SNAKE]` placeholder remaining — the tz-match is NOT re-checked here; it ran against the assembled config at landing time). If the gate fails, the install action MUST exit NON-ZERO with a loud "NOT installed" message (distinct from a successful install) BEFORE the bundle POST. The check NAMES the failing invariant (never the PII values). The [`ld-config` verify check](#verify) cross-checks the same structural gate at verify time. Per-field requirements are enforced at runtime by each bundle, not here.
...
2. **ld-config present, well-formed, and passes the structural gate.** Does `<app_support>/agent-runtime/runtime/ld/config.json` exist, parse as JSON, AND pass the structural gate — `calendar.sources` a non-empty array with non-blank `account`s, `family.owner.{name,imessage}` non-blank, and no string value left as a bare `[UPPER_SNAKE]` placeholder (a real value that merely contains a bracketed token being fine)? Expected: yes — a gate-passing config is the SEED's single source of truth for "install complete." [ld-config is landed](#ld-config-is-landed) enforces the same structural gate at install time (refuses to POST bundles otherwise); this verify step is the cross-check that the gate held. The tz-match is NOT checked here — it is the assembly-time regression guard (a preserved/supplied config may legitimately carry a non-host zone), so the install post gate and this verify step stay in agreement. Per-field requirements are enforced at runtime by each bundle. The values are PII so only the check name prints, never the contents.
