...
5. **Scheduled crons registered.** The install drives the agent to register the three crons (see [Scheduled crons are registered](#scheduled-crons-are-registered)). Confirm — authoritatively — via Plow `cron action=list`, or host-side by reading (never editing) `<app_support>/agent-runtime/gateway/cron/jobs.json` and checking `ld-morning-updates`, `ld-morning-triage`, and `ld-weekly-digest` are present and `enabled` with the schedules above (`ld-calendar-nudge` / `ld-weather` auto-recur via `scheduled/` and need no cron). Because registration rides an **async agent turn**, this is verified by bounded poll, not a synchronous host gate, so it is NOT in host-only `ref/verify.sh`; if absent after the timeout, re-send the setup message (the live gateway view is authoritative — `jobs.json` is only an observation). Expected: all three present and enabled.
