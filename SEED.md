...
- **The install action MUST also materialize the gate-passing `ld-config` into each active agent container's per-container config dir** — `<app_support>/containers/<runtime-id>/config/ld/config.json` for every `containers/index.json` entry with `role: agent` and `enabled: true` (id lowercased). This is REQUIRED, not optional: the agent container mounts its `/config/runtime` from that per-container dir (`app/vm/provision-runtime.sh`: `--volume=/containers/<RUNTIME_ID>/config:/config/runtime`), NOT from `agent-runtime/runtime`. The canonical `agent-runtime/runtime/ld/config.json` copy is what host tooling and [`ref/verify.sh`](#verification) read, but the **running agent and the scheduled runner read `/config/runtime/ld/config.json`** — so without this step every `ld-*` job (the `cron`-driven trio AND the auto-activated `scheduled/` weather/nudge) fails on first fire with a missing-config error, even though the host-side gate passed.
...
- **ld-config materialization is plowd's job, long-term.** This SEED copies `ld-config` into each agent container's per-container `config/ld/` because the container mounts `/config/runtime` from `containers/<runtime-id>/config`, not from the canonical `agent-runtime/runtime` (where the SEED + `plow-prod-prime` treat it as source-of-truth). That couples the SEED to plowd's container layout and is fragile across container re-creation (a new `runtime-id` on a Plow update gets no config until the SEED re-runs). The durable fix is **plowd materializing the canonical `agent-runtime/runtime/ld/config.json` into every agent container's `/config/runtime` at provision time** (it already does this for `/config/secrets` and `/config/gateway`); then this SEED drops the per-container copy and `plow-prod-prime`'s mapping becomes true again. Tracked as a plowd change.
