...
- `ref/install-bundles.sh` surfaces this as a loud post-install note (it cannot run the verb itself).
...
5. **Scheduled crons registered (agent-side).** Ask Plow `cron list` and confirm `ld-morning-updates`, `ld-morning-triage`, and `ld-weekly-digest` appear (`ld-calendar-nudge` / `ld-weather` auto-recur via `scheduled/` and need no cron). This is a **runtime, agent-side** check — it is NOT in `ref/verify.sh` (host-only); it confirms the [Scheduled crons are registered](#scheduled-crons-are-registered) step actually ran. Expected: all three present.
...
- **Cron registration is now a first-class [Action](#scheduled-crons-are-registered)** (with the exact per-bundle `cron action=add` params) and a [Verification](#verification) step, rather than only a post-install note. The residual gap: registering the three crons is an agent-side runtime verb the host install cannot drive, so it remains the install's one human-or-orchestrator step. The often-suggested "just move them to the `scheduled/` entrypoint" is **not** a drop-in: that runner executes *deterministic* JS (like `ld-weather`), whereas these three need an LLM turn — converting them would require a scheduled-job primitive that spawns an agent session, which plowd does not expose today. The real v2 is that primitive (a plowd change), not a bundle reshuffle.
