...
- **Required vs optional fields.** The install/verify gate blocks ONLY on REQUIRED fields the bundles cannot function without:
  - `family.owner.name`, `family.owner.imessage`, and `calendar.sources` with at least ONE row where EVERY present row has a real `account` (non-empty, not an `[UPPER_SNAKE]` placeholder). Every source is fetched at the first scheduled tick, so a single empty/placeholder `account` blocks — it would be a bogus fetch target.
...
- The supplied-config path (when `LD_CONFIG_SRC` is set) MUST run this required-field gate against the fetched bytes BEFORE the atomic `mv` — a valid-JSON-but-incomplete supplied config MUST NOT land, or a re-run would short-circuit on the bad file and silently ignore a corrected `LD_CONFIG_SRC` retry. The vendored-example path still lands placeholders for manual editing.
- After landing (or detecting an existing) `ld-config`, the install action MUST gate the bundle POST on REQUIRED-field placeholders only: `family.owner.name`, `family.owner.imessage`, and `calendar.sources` with at least one row where every present row carries a real (non-empty, non-placeholder) `account`. If any REQUIRED field is still an `[UPPER_SNAKE]` placeholder or empty, the install action MUST exit NON-ZERO with a loud "missing required values / NOT installed" message (distinct from a successful install) BEFORE the bundle POST. OPTIONAL fields (`[PARTNER_*]`, `[FAMILY_PERSON_*]`, `[FAMILY_CALENDAR_ID]`, `[LONG_LEAD_TYPE]`) MUST NOT block. The [`ld-config` verify check](#verify) cross-checks the same required-only gate at verify time. Field NAMES (never the PII values) appear in the message.
